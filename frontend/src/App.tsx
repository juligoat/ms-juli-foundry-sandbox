...
const App: React.FC = () => {
  ...
  useEffect(() => {
    setInputModeRef(inputMode);
  }, [inputMode, setInputModeRef]);
  ...
  const agentDisplayName =
    settings.mode === "agent" ? settings.agentName || "Voice Assistant" : settings.model || "Voice Assistant";
  const formatName = (name: string) => name.replace(/([a-z])([A-Z])/g, "$1 $2").replace(/[-_]/g, " ");

  return (
    <FluentProvider
      theme={resolvedTheme === "dark" ? voiceLiveDarkTheme : voiceLiveLightTheme}
      style={{ height: "100vh" }}
    >
              ...
              <>
                ...
                <div style={agentDetailsStyle}>
                  <Text as="h1" weight="semibold" size={400}>
                    {formatName(agentDisplayName)}
                  </Text>
                  <Text size={300} style={{ color: "var(--fg-2)" }}>
                    {settings.mode === "agent"
                      ? settings.project
                        ? `${formatName(agentDisplayName)} with ${settings.project} project.`
                        : "Voice agent ready to assist."
                ...
                <div style={idlePanelStyle}>
                  ...
                  <div style={idleSubtitleStyle}>Talk like you would to a person. The agent listens and responds.</div>
                  <Button
                    appearance="primary"
                    shape="circular"
                    size="medium"
                  ...
                  {startDisabled && !isLocked && (
                    <Text size={200} style={{ color: "var(--fg-2)" }}>
                      Open Settings (···) to configure Agent Name and Project
                    </Text>
                  )}
                </div>
              </>
...
const idleSubtitleStyle: React.CSSProperties = {
  fontSize: "14px",
  color: "var(--fg-2)",
  maxWidth: "250px",
  textAlign: "center",
...
const textChatContainerStyle: React.CSSProperties = {
  display: "flex",
  flexDirection: "column",
  width: "100%",
  height: "100%",
...
const textActionBarStyle: React.CSSProperties = {
  display: "flex",
  gap: "8px",
  alignItems: "center",
  padding: "12px 0",
