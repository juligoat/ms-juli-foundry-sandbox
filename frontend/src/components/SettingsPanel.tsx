...
function isAzureSpeechTranscription(model: string, transcribeModel: string): boolean {
  return (
    PHI_MULTIMODAL_MODELS.includes(model) ||
...
function buildAzureSpeechLanguageOptions(locales: string[]): { value: string; label: string }[] {
  ...
  return [{ value: "", label: "Auto-detect (multilingual)" }, ...locales.map(l => ({ value: l, label: l }))];
}
    ...
    <>
      ...
      <div style={panelStyle}>
        ...
        <div style={fieldStyle}>
          ...
          <Select value={theme} onChange={(_e, data) => onThemeChange(data.value as ThemePreference)}>
        ...
        <div style={fieldStyle}>
          ...
          {settings.voiceType === "openai" ? (
            <Select value={settings.voice} onChange={(_e, data) => onUpdate({ voice: data.value })}>
              {OPENAI_VOICES.map(v => (
            ...
            <>
              <Select
                value={AZURE_DRAGON_HD_VOICES.includes(settings.voice) ? settings.voice : CUSTOM_VOICE_SENTINEL}
                onChange={(_e, data) => {
              ...
              >
                <optgroup label="DragonHD (Recommended)">
                  {AZURE_DRAGON_HD_VOICES.map(v => (
                    <option key={v} value={v}>
                      {v.replace(":DragonHDLatestNeural", "")}
                    </option>
                  ))}
                </optgroup>
        ...
        {settings.mode === "model" ? (
          <>
            ...
            <div style={fieldStyle}>
              <Label weight="semibold">Temperature: {settings.temperature.toFixed(1)}</Label>
              <Slider
                min={0}
                max={1}
                step={0.1}
            ...
            <div style={fieldStyle}>
              ...
              <Select value={settings.inputLanguage} onChange={(_e, data) => onUpdate({ inputLanguage: data.value })}>
          ...
          <>
            ...
            <div style={fieldStyle}>
              <Label weight="semibold">
                {settings.greetingType === "llm" ? "Greeting Instruction" : "Greeting Text"}
              </Label>
              <Textarea
        ...
        {(() => {
          const isRealtimeModelMode =
            settings.mode === "model" && [...GPT_MULTIMODAL_MODELS, ...PHI_MULTIMODAL_MODELS].includes(settings.model);
          return (
            <div style={checkboxRowStyle}>
              <Checkbox
        ...
        {settings.interimResponse && (
          <>
            ...
            {settings.interimTriggerLatency && (
              <div style={fieldStyle}>
                <Label weight="semibold">Latency Threshold: {settings.interimLatencyMs}ms</Label>
                <Slider
                  min={50}
                  max={2000}
                  step={50}
        ...
        <div style={fieldStyle}>
          ...
          <Select value={settings.vadType} onChange={(_e, data) => onUpdate({ vadType: data.value as any })}>
