...
export function useVoiceSession() {
  ...
  const startSession = useCallback(async () => {
    ...
    try {
      ...
      ws.onopen = () => {
        ...
        const effectiveInterim =
          settings.interimResponse && !(settings.mode === "model" && REALTIME_MODELS.includes(settings.model));
  ...
  const sendTextMessage = useCallback(
    (text: string) => {
      ...
      setTranscripts(prev => [...prev, { role: "user", text: text.trim(), isFinal: true, timestamp: Date.now() }]);
  ...
  return {
    ...
    sendTextMessage,
    configLoaded,
    setInputModeRef: useCallback((mode: "voice" | "text") => {
